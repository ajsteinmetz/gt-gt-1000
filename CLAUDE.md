...
## This repo: gt-gt-1000 (GT 1000, First-Year Seminar)

- Course: GT 1000 First-Year Seminar, Physics, section D03, Fall 2026. Audience: first-year Physics, Astrophysics, and Applied Physics majors. Forked from `gt-phys-4604`.
- Course-page subtitle: `"GT 1000: First-Year Seminar, Physics"`. Lecture subtitle: `"GT 1000 &middot; Lecture N"`.
- Lecture footer: `"GT 1000 &middot; Fall 2026 &middot; Section D03 &middot; Georgia Institute of Technology"`.
- Grading is four equal 25% components: participation/attendance, **Team Project**, **Academic Planning**, **Career Readiness**. Each graded component has an overview page in `assignments/` (`team-project.qmd`, `academic-planning.qmd`, `career-readiness.qmd`) with a Deliverables table; each deliverable is its own page in `assignments/deliverables/<kebab-case-name>.qmd`. Deliverable titles are plain names (`title: "Degree Map"`).
- Course facts (dates, weights, policies): `course-files/syllabus-latex/gt-1000-syllabus-fall26.tex` is the source of truth for the printed syllabus; the Course Schedule in the root `index.qmd` is the live schedule and wins if they differ. Keep `course-files/syllabus.qmd` consistent with both.
- `planning/` holds instructor research notes (e.g. deadline sources cited in lecture notes). It is gitignored and excluded from rendering.
- `submissions/` holds student work. It is gitignored and must never be committed, published, or quoted outside this machine.
- Student photo submissions and derived collage tiles under `lectures/figs/fluff/` (`submissions/`, `collage/`) are gitignored for the same reason.
